...
import { getMockSchemes, getGroupDetails, addMockScheme, updateMockSchemePayment, getUniqueGroupNames, reopenMockScheme, deleteFullMockScheme, getArchivedMockSchemes, unarchiveMockScheme, archiveMockScheme, getArchivedGroups, unarchiveMockGroup, deleteFullMockGroup, getArchivedPaymentsForAllSchemes, unarchiveMockPayment, deleteFullMockPayment, type ArchivedPaymentAugmented } from '@/lib/mock-data';
import type { Scheme, PaymentMode, GroupDetail, Payment, SchemeStatus, MockGroup } from '@/types/scheme';
import { formatDate, formatCurrency, getPaymentStatus, generateId, getSchemeStatus } from '@/lib/utils';
import * as XLSX from 'xlsx';
// Tabs components are not needed for a dedicated page, but keeping other UI components
...
// RadioGroup might not be used by DataManagementTabContent, verify later if needed
...
import { formatISO, format as formatDateFns, parseISO, isValid as isValidDate, isBefore, subDays, } from 'date-fns';
...
// Renamed from DataManagementTabContent
export default function DataManagementPage() {
  ...
  const [isDeletingClosedSchemes, setIsDeletingClosedSchemes] = useState(false); // Not used in UI, but kept for consistency if logic exists
  const [isArchivingSelectedSchemes, setIsArchivingSelectedSchemes] = useState(false);
  ...
  const handleExportAllToExcel = () => {
    ...
    try {
      ...
      // Define exportToExcel locally for this component
      const localExportToExcel = (sheets: { name: string; data: any[][] }[], fileName: string) => {
  ...
  const handleArchiveSelectedSchemes = async () => {
    ...
    for (const schemeId of selectedClosedSchemeIds) {
      const schemeInList = completedSchemes.find(s => s.id === schemeId);
      if (schemeInList) {
        const actualSchemeToArchive = currentSystemSchemes.find(s => s.id === schemeId);
        if (actualSchemeToArchive && actualSchemeToArchive.status === 'Closed') {
            const result = archiveMockScheme(schemeId);
            if (result) {
              successCount++;
            } else {
              errorCount++;
  ...
  const handleInitiateDeleteArchivedSchemes = () => {
    ...
    const info = selectedArchivedSchemeIds.map(id => {
      const scheme = archivedSchemesList.find(s => s.id === id);
    ...
    setShowDeleteArchivedConfirmDialog(true);
  };

  const handleConfirmDeleteSelectedArchivedSchemes = async () => {
    if (selectedArchivedSchemeIds.length === 0) return;
    setIsDeletingArchivedSchemes(true);
    let successCount = 0;
    let errorCount = 0;
  ...
  const handleInitiateDeleteArchivedTransactions = () => {
    ...
    const info = selectedArchivedPaymentIds.map(id => {
      ...
      return t || { id, schemeId: 'Unknown', customerName: 'Unknown', monthNumber: 0, archivedDate: undefined, amountExpected: 0 };
    }).map(t => ({ paymentId: t.id, schemeId: t.schemeId, customerName: t.customerName, monthNumber: t.monthNumber }));
    setArchivedTransactionsPendingDeletionInfo(info as any);
  ...
  return (
    // Removed the outer div with mt-6, as this will be a page now. Page layout can handle margins.
    <div className="flex flex-col gap-8">
      {/* Page Title (Optional - Can be handled by AppLayout) */}
      ...
      <Card>
        <CardHeader>
          <CardTitle className="font-headline flex items-center gap-2">
            <ArchiveRestore className="h-5 w-5 text-primary opacity-80" />
      ...
      <Card>
        <CardHeader>
          <CardTitle className="font-headline flex items-center gap-2">
            <Users2Icon className="h-5 w-5 text-primary opacity-80" />
                ...
                <ScrollArea className="max-h-40 mt-2 border rounded-md p-2">
                  <ul className="list-disc pl-5 text-sm ">
                    {archivedTransactionsPendingDeletionInfo.map(t => (
                      <li key={t.id}> {/* Assuming t.id is paymentId here */}
